// This file was generated by the Gtk# code generator.
// Any changes made will be lost if regenerated.

namespace PackageKit {

	using System;
	using System.Runtime.InteropServices;

#region Autogenerated code
	public class Role {

		[DllImport("libpackagekit-glib.dll")]
		static extern IntPtr pk_role_enum_to_string(int role);

		public static string EnumToText(PackageKit.RoleEnum role) {
			IntPtr raw_ret = pk_role_enum_to_string((int) role);
			string ret = GLib.Marshaller.Utf8PtrToString (raw_ret);
			return ret;
		}

		[DllImport("libpackagekit-glib.dll")]
		static extern ulong pk_role_bitfield_from_string(IntPtr roles);

		public static ulong BitfieldFromText(string roles) {
			IntPtr native_roles = GLib.Marshaller.StringToPtrGStrdup (roles);
			ulong raw_ret = pk_role_bitfield_from_string(native_roles);
			ulong ret = raw_ret;
			GLib.Marshaller.Free (native_roles);
			return ret;
		}

		[DllImport("libpackagekit-glib.dll")]
		static extern int pk_role_enum_from_string(IntPtr role);

		public static PackageKit.RoleEnum EnumFromText(string role) {
			IntPtr native_role = GLib.Marshaller.StringToPtrGStrdup (role);
			int raw_ret = pk_role_enum_from_string(native_role);
			PackageKit.RoleEnum ret = (PackageKit.RoleEnum) raw_ret;
			GLib.Marshaller.Free (native_role);
			return ret;
		}

		[DllImport("libpackagekit-glib.dll")]
		static extern IntPtr pk_role_bitfield_to_string(ulong roles);

		public static string BitfieldToText(ulong roles) {
			IntPtr raw_ret = pk_role_bitfield_to_string(roles);
			string ret = GLib.Marshaller.PtrToStringGFree(raw_ret);
			return ret;
		}

#endregion
	}
}
