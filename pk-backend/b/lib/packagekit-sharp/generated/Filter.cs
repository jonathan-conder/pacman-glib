// This file was generated by the Gtk# code generator.
// Any changes made will be lost if regenerated.

namespace PackageKit {

	using System;
	using System.Runtime.InteropServices;

#region Autogenerated code
	public class Filter {

		[DllImport("libpackagekit-glib.dll")]
		static extern IntPtr pk_filter_enum_to_string(int filter);

		public static string EnumToText(PackageKit.FilterEnum filter) {
			IntPtr raw_ret = pk_filter_enum_to_string((int) filter);
			string ret = GLib.Marshaller.Utf8PtrToString (raw_ret);
			return ret;
		}

		[DllImport("libpackagekit-glib.dll")]
		static extern ulong pk_filter_bitfield_from_string(IntPtr filters);

		public static ulong BitfieldFromText(string filters) {
			IntPtr native_filters = GLib.Marshaller.StringToPtrGStrdup (filters);
			ulong raw_ret = pk_filter_bitfield_from_string(native_filters);
			ulong ret = raw_ret;
			GLib.Marshaller.Free (native_filters);
			return ret;
		}

		[DllImport("libpackagekit-glib.dll")]
		static extern int pk_filter_enum_from_string(IntPtr filter);

		public static PackageKit.FilterEnum EnumFromText(string filter) {
			IntPtr native_filter = GLib.Marshaller.StringToPtrGStrdup (filter);
			int raw_ret = pk_filter_enum_from_string(native_filter);
			PackageKit.FilterEnum ret = (PackageKit.FilterEnum) raw_ret;
			GLib.Marshaller.Free (native_filter);
			return ret;
		}

		[DllImport("libpackagekit-glib.dll")]
		static extern IntPtr pk_filter_bitfield_to_string(ulong filters);

		public static string BitfieldToText(ulong filters) {
			IntPtr raw_ret = pk_filter_bitfield_to_string(filters);
			string ret = GLib.Marshaller.PtrToStringGFree(raw_ret);
			return ret;
		}

#endregion
	}
}
